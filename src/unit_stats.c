...
static const fighter_bay_view_t k_fighter_types(unit_type_t type){
    switch (type) {
        ...
        case TYPE_CARRIER:   return (fighter_bay_view_t){.capacity = 3, .current = 0, .sq_types = {TYPE_BOMBER, TYPE_BOMBER, TYPE_FIGTER, TYPE_BOMBER, TYPE_FIGTER, TYPE_FIGTER}};
        case TYPE_DESTROYER: return (fighter_bay_view_t){.capacity = 2, .current = 0, .sq_types = {TYPE_FIGTER, TYPE_FIGTER, TYPE_BOMBER, TYPE_FIGTER, DUMMY, DUMMY}};
...
unit_stats_t unit_stats_for_type(unit_type_t type) {
    switch (type) {
        ...
        case TYPE_FIGTER:    return (unit_stats_t){.hp = 20,    .sh = 0,    .en = 20,   .sp = 5,    .si = 1,    .dr = 10,   .ba = weapon_loadout_for_unit_type(type),   .fb = k_fighter_types(type)};
